...
const VER = 5;
// Deployment-shaped hostname of the DEDICATED artifact project. The generated
...
test("missing or invalid artifact origin fails closed with opaque 404", async () => {
  for (const bad of [
    undefined,
    "",
    "http://penalty444-unity-staging-abc123.vercel.app", // non-https
    "https://penalty444-unity-staging.vercel.app", // the project/production alias
    "https://penalty444-platform-at1y-abc123-team.vercel.app", // unrelated project
    "https://penalty444-unity-staging-.vercel.app", // empty deployment suffix
    `${ORIGIN}/releases/x`, // non-root path
  ]) {
    const t = stubTransport();
