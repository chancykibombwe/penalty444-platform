...
export default function MatchRoomPanel({ roomCode }: { roomCode: string }) {
  ...
  return (
    <>
      ...
      <div
        className={`relative z-10 main-container flex flex-col px-2 py-1 gap-0.5 text-white h-svh max-h-svh ${matchEnded ? "overflow-y-auto" : "overflow-hidden"} ${
          screenEffect === "GOAL"
            ? "zoom-impact ring-2 ring-green-400/80 shadow-[0_0_40px_rgba(34,197,94,0.55)]"
            : screenEffect === "SAVE"
              ? "shake-impact ring-2 ring-sky-500 shadow-[0_0_36px_rgba(56,189,248,0.55)] match-screen-shake"
              : screenEffect === "DRAW"
                ? "soft-impact ring-1 ring-zinc-300/90 shadow-[0_0_28px_rgba(212,212,216,0.45)]"
                : isSuddenDeath
                  ? "border border-yellow-400/45 bg-yellow-500/10 shadow-[0_0_28px_rgba(234,179,8,0.14)]"
                  : isLateGame
                    ? "border border-red-400/35 bg-red-500/5 shadow-[0_0_22px_rgba(248,113,113,0.08)]"
                    : ""
        }`}
      >
      ...
      <section
        className="relative shrink-0 h-[34vh] sm:h-[44vh] md:h-[52vh] w-full overflow-hidden rounded-2xl border border-zinc-800 bg-black/60"
        aria-hidden
      >
        ...
        <section className="relative shrink-0 flex flex-col rounded-2xl border border-zinc-800 bg-zinc-900/95 px-2 py-2" style={{height:"22vh"}}>
          {revealStage === "LOCKED" && !isRevealLocked ? (
            <div
              ...
              aria-live="polite"
            >
              <p className="p444-pick-locked-pulse text-[10px] font-black uppercase tracking-[0.34em] text-cyan-300">
                Pick locked · {disconnectCountdown !== null ? "Opponent disconnected" : "Waiting for opponent…"}
              </p>
            </div>
          ) : null}
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {(hasSubmittedPick || myPick) && !isRevealLocked && disconnectCountdown === null ? (
              ...
              {opponentStatus && !isRevealLocked ? (
                <span className="text-[9px] font-bold text-amber-300">
                  <span className="inline-block h-1 w-1 animate-pulse rounded-full bg-amber-300 mr-0.5" aria-hidden />
                  Opp locked
                </span>
              ) : null}
            </div>
          ...
          <div className="mt-1 flex-1 min-h-0 grid grid-cols-3 gap-1.5">
            {LANES.map((lane) => (
              ...
              >
                <span className="text-sm leading-none">{laneEmoji(lane)}</span>
      ...
      <div
        className={`shrink-0 flex items-center gap-3 rounded-xl border px-2 py-1 transition-all duration-300 ${resultStyle(
          shownResult?.result
        )} ${
          revealStage === "REVEALED"
      ...
      >
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 shrink-0">
          {shownResult?.round ? `R${shownResult.round}` : "Result"}
        </p>
        <p
          className={`text-xs font-black transition-all duration-300 ${resultHeadlineClass(
            shownResult?.result,
            revealStage
          )}`}
        >
          {revealStage === "REVEALING"
            ? tensionCountdown !== null
              ...
              : "Locked"
            : shownResult?.result
              ? `${resultEmoji(shownResult.result)} ${resultLabel(shownResult?.result)}`
              : "Waiting for result"}
        </p>
        {shownResult?.kickerPick || shownResult?.keeperPick ? (
          <p className="ml-auto text-[10px] text-white/50 shrink-0">
            K:{shownResult?.kickerPick ?? "—"} · G:{shownResult?.keeperPick ?? "—"}
          </p>
        ) : null}
        {revealStage === "REVEALED" && !matchEnded && shownResult?.result ? (
          <span className="ml-auto text-[9px] font-black uppercase tracking-[0.18em] text-white/60 shrink-0">
            <span className="match-waiting-dots" aria-hidden>
              <span className="match-waiting-dot" />
              <span className="match-waiting-dot" />
              <span className="match-waiting-dot" />
      ...
      {matchEnded && matchEndOutcome ? (
        <section
          className={`shrink-0 rounded-2xl border p-3 shadow-2xl sm:p-5 sm:rounded-3xl md:rounded-[2rem] md:p-10 ${
        ...
        >
          ...
          <h2
            className={`mt-1 break-words text-3xl font-black uppercase tracking-tight sm:text-4xl sm:mt-2 md:mt-3 md:text-6xl ${
          ...
          <p
            className={`mt-1 max-w-xl text-sm font-semibold leading-snug sm:mt-3 sm:text-base md:text-lg ${
              isFinalTournamentMatch && matchEndOutcome === "victory"
                ? "text-yellow-100"
                : matchEndOutcome === "victory"
                  ? "text-emerald-100/90"
          ...
          <div
            className={`mt-3 rounded-xl border px-3 py-3 sm:mt-6 sm:rounded-2xl sm:px-6 sm:py-6 md:px-8 md:py-8 ${
              matchEndOutcome === "victory"
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-3 text-center sm:gap-4 md:mt-4 md:gap-8">
              <div className="min-w-[5rem] sm:min-w-[7rem]">
                ...
                <p className="mt-1 text-4xl font-black tabular-nums text-white sm:text-5xl sm:mt-1 md:text-8xl">
                  {myScore}
                </p>
              </div>
              <span className="text-2xl font-black text-zinc-600 sm:text-3xl md:text-5xl">
                —
              </span>
              <div className="min-w-[5rem] sm:min-w-[7rem]">
                <p className="truncate text-xs font-semibold text-zinc-400 sm:text-sm">
                ...
                <p className="mt-1 text-4xl font-black tabular-nums text-white sm:text-5xl md:text-8xl">
                  {opponentScore}
                </p>
              </div>
            </div>
          ...
          <div className="mt-3 flex flex-col gap-2 sm:mt-6 sm:gap-3 md:flex-row md:flex-wrap md:items-center">
